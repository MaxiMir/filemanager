$(function () {
    const encodeSearchText = getEncodeSearchText();
    ...
    if (encodeSearchText) {
        showSearchPhrase(encodeSearchText);
    }
...
function getEncodeSearchText() {
    const uri = $(location).attr('href');
    const uriData = uri.split('#find=');

    return uriData.length === 2 ? uriData[1] : false;
}

function showSearchPhrase(encodeSearchText) {
    const decodeSearchText = decodeURIComponent(escape(window.atob(encodeSearchText)));

    editor.find(decodeSearchText);
